...
from groq import Groq  # Import Groq API yang benar
...
# Judul utama
st.title("Asesmen Minat Siswa")
st.subheader("Front End, Back End, Full Stack, atau Game Development")

# Petunjuk
with st.expander("Petunjuk", expanded=True):
    st.write("""
    Jawablah 10 pertanyaan berikut dengan memilih bidang yang paling menarik minat Anda.
    Setelah menjawab semua soal, Anda bisa menekan tombol untuk melihat hasil visualisasi.
    """)
...
# Fungsi untuk mendapatkan rekomendasi dari Groq API
def get_rekomendasi(data_minat, nama, kelas, nomor_absen):
    prompt = (
        f"Data minat siswa: {data_minat}. "
        f"Nama: {nama}, Kelas: {kelas}, Nomor Absen: {nomor_absen}. "
        "Berikan rekomendasi karir berdasarkan minat ini beserta arahan belajar yang tepat dalam bahasa Indonesia."
    )
    try:
        # Pemanggilan Groq API
        response = client.chat.completions.create(
            model="llama3-8b-8192",  # Gunakan model llama3-8b-8192
            messages=[
                {"role": "system", "content": "Anda adalah asisten karir."},
                {"role": "user", "content": prompt}
            ],
            # max_tokens=300
        )
        return response.choices[0].message.content  # Ambil hasil dari respons Groq API
    
    except Exception as e:
        st.error(f"Terjadi kesalahan saat mengambil rekomendasi dari Groq API: {str(e)}")
        return None
...
if st.button("Tampilkan Hasil"):
    ...
    # Mendapatkan rekomendasi karir dari Groq API
    rekomendasi = get_rekomendasi(data_minat, nama, kelas, nomor_absen)
